...
import { getD20Chart } from "../../api/Charts";
...
const D20 = ({ currentLanguage: propCurrentLanguage, onLanguageChange }) => {
  const { user } = useContext(AppContext);
  const [d20Data, setD20Data] = useState(null);
  const [isLoadingD20, setIsLoadingD20] = useState(false);
  ...
  //   document.title = `D20 Chart - ${userName}`;
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    setIsLoadingD20(true);
    try {
      const cacheKey = `d3Data_${lang}`;
      const storedData = localStorage.getItem(cacheKey);

      if (storedData && !forceRefresh) {
        try {
          const parsedData = JSON.parse(storedData);
          setD20Data(parsedData);
          setCurrentLanguage(lang);
          setIsLoadingD20(false);
          return;
        } catch (err) {
      ...
      const response = await getD20Chart(userData.user._id, apiParams);

      if (response && response.success) {
        setD20Data(response);
        localStorage.setItem(cacheKey, JSON.stringify(response));
        // toast.success("D20 chart fetched successfully", {
        //   position: "top-right",
        //   autoClose: 1000,
        // });
        setCurrentLanguage(lang);
      ...
      console.error("Failed to fetch D20 chart:", err);
      toast.error("Failed to load D20 chart. Please try again.");
    } finally {
      setIsLoadingD20(false);
    }
  };
  ...
  return (
    <div className="chart-item">
      <h4 className="chart-title">Vimshamsha (Religious Inclinations - D20)</h4>
      <div>
        {d20Data?.charts && (
          <SVGRenderer svgString={d20Data?.charts} className="chart-image" />
        )}
      </div>
    </div>
  );

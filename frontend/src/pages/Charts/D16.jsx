...
import { getD16Chart } from "../../api/Charts";
...
const D16 = ({ currentLanguage: propCurrentLanguage, onLanguageChange }) => {
  const { user } = useContext(AppContext);
  const [d16Data, setD16Data] = useState(null);
  const [isLoadingD16, setIsLoadingD16] = useState(false);
  ...
  //   document.title = `D16 Chart - ${userName}`;
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    setIsLoadingD16(true);
    try {
      const cacheKey = `d16Data_${lang}`;
      const storedData = localStorage.getItem(cacheKey);

      if (storedData && !forceRefresh) {
        try {
          const parsedData = JSON.parse(storedData);
          setD16Data(parsedData);
          setCurrentLanguage(lang);
          setIsLoadingD16(false);
          return;
        } catch (err) {
      ...
      const response = await getD16Chart(userData.user._id, apiParams);

      if (response && response.success) {
        setD16Data(response);
        localStorage.setItem(cacheKey, JSON.stringify(response));
        // toast.success("D16 chart fetched successfully", {
        //   position: "top-right",
        //   autoClose: 1000,
        // });
        setCurrentLanguage(lang);
      ...
      console.error("Failed to fetch D16 chart:", err);
      toast.error("Failed to load D16 chart. Please try again.");
    } finally {
      setIsLoadingD16(false);
    }
  };
  ...
  return (
    <div className="chart-item">
      <h4 className="chart-title">Shodasamsha (Vehicles - D16)</h4>
      <div>
        {d16Data?.charts && (
          <SVGRenderer svgString={d16Data?.charts} className="chart-image" />
        )}
      </div>
    </div>
  );

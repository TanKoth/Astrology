...
import { getSarvashtakavargaChart, getD1Chart } from "../../api/Charts";
...
const SarvashtakvargaChart = ({
  currentLanguage: propCurrentLanguage,
  onLanguageChange,
}) => {
  const { user } = useContext(AppContext);
  const [sarvashtakvargaData, setSarvashtakvargaData] = useState(null);
  const [isLoadingSarvashtakvarga, setIsLoadingSarvashtakvarga] =
    useState(false);
  ...
  //   document.title = `Sarvashtakavarga Chart - ${userName}`;
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    setIsLoadingSarvashtakvarga(true);
    try {
      // Make cache key user-specific
      const cacheKey = `sarvashtakvargaData_${user._id}_${lang}`;
      const storedData = localStorage.getItem(cacheKey);

      if (storedData && !forceRefresh) {
        try {
          const parsedData = JSON.parse(storedData);
          setSarvashtakvargaData(parsedData);
          setCurrentLanguage(lang);
          setIsLoadingSarvashtakvarga(false);
          return;
        } catch (err) {
          console.error("Error parsing sarvashtakvarga data:", err);
        }
      }
      ...
      // Fetch both Sarvashtakavarga and D1 chart data
      const [sarvashtakavargaResponse, d1Response] = await Promise.all([
        getSarvashtakavargaChart(userData.user._id, apiParams),
        getD1Chart(userData.user._id, apiParams),
      ]);

      if (sarvashtakavargaResponse && sarvashtakavargaResponse.success) {
        //console.log("Sarvashtakavarga API Response:", sarvashtakavargaResponse);

        // Parse Sarvashtakavarga SVG to extract values
        const parser = new DOMParser();
        const apiSvgDoc = parser.parseFromString(
          sarvashtakavargaResponse.data,
          "image/svg+xml"
        );

        // Extract ashtakavarga values from API response
        const ashtakavargaValues = [];

        // Try different selectors to find the values
        let valueElements = apiSvgDoc.querySelectorAll(
          'text[class*="ashtakavarga-score"]'
        );

        if (valueElements.length === 0) {
          // Try alternative selectors
          valueElements = apiSvgDoc.querySelectorAll(
            'text[style*="font-size:17px"]'
          );
        }

        if (valueElements.length === 0) {
          // Try to get all text elements and filter numeric values
          const allTextElements = apiSvgDoc.querySelectorAll("text");
          allTextElements.forEach((textElement) => {
            const value = parseInt(textElement.textContent.trim());
            if (!isNaN(value) && value >= 0 && value <= 50) {
              // Reasonable range for ashtakavarga values
              ashtakavargaValues.push(value);
            }
          });
        } else {
          valueElements.forEach((textElement) => {
            const value = parseInt(textElement.textContent.trim());
            if (!isNaN(value)) {
              ashtakavargaValues.push(value);
            }
        ...
        // Ensure we have 12 values, pad with 0 if needed
        while (ashtakavargaValues.length < 12) {
          ashtakavargaValues.push(0);
        }

        //console.log("Extracted Sarvashtakavarga Values:", ashtakavargaValues);
        ...
        <!-- Red circles around Sarvashtakavarga values -->
        <circle cx="167" cy="77" r="12" fill="none" stroke="red" stroke-width="2"/>
        <circle cx="234" cy="51" r="12" fill="none" stroke="red" stroke-width="2"/>
        <circle cx="278" cy="85" r="12" fill="none" stroke="red" stroke-width="2"/>
        <circle cx="226" cy="152" r="12" fill="none" stroke="red" stroke-width="2"/>
        <circle cx="276" cy="228" r="12" fill="none" stroke="red" stroke-width="2"/>
        <circle cx="226" cy="282" r="12" fill="none" stroke="red" stroke-width="2"/>
        <circle cx="167" cy="227" r="12" fill="none" stroke="red" stroke-width="2"/>
        <circle cx="91" cy="281" r="12" fill="none" stroke="red" stroke-width="2"/>
        <circle cx="36" cy="227" r="12" fill="none" stroke="red" stroke-width="2"/>
        <circle cx="86" cy="151" r="12" fill="none" stroke="red" stroke-width="2"/>
        <circle cx="36" cy="76" r="12" fill="none" stroke="red" stroke-width="2"/>
        <circle cx="99" cy="55" r="12" fill="none" stroke="red" stroke-width="2"/>

        <!-- Dynamic Sarvashtakavarga values -->
        <text x="160" y="80" style="font-family:Arial;font-size:12px;fill:#2c5aa0;font-weight:bold">${
          ashtakavargaValues[6] || 0
        }</text>
        <text x="228" y="54" style="font-family:Arial;font-size:12px;fill:#2c5aa0;font-weight:bold">${
          ashtakavargaValues[11] || 0
        }</text>
        <text x="271" y="88" style="font-family:Arial;font-size:12px;fill:#2c5aa0;font-weight:bold">${
          ashtakavargaValues[10] || 0
        }</text>
        <text x="220" y="155" style="font-family:Arial;font-size:12px;fill:#2c5aa0;font-weight:bold">${
          ashtakavargaValues[9] || 0
        }</text>
        <text x="270" y="230" style="font-family:Arial;font-size:12px;fill:#2c5aa0;font-weight:bold">${
          ashtakavargaValues[8] || 0
        }</text>
        <text x="220" y="285" style="font-family:Arial;font-size:12px;fill:#2c5aa0;font-weight:bold">${
          ashtakavargaValues[7] || 0
        }</text>
        <text x="160" y="230" style="font-family:Arial;font-size:12px;fill:#2c5aa0;font-weight:bold">${
          ashtakavargaValues[6] || 0
        }</text>
        <text x="85" y="285" style="font-family:Arial;font-size:12px;fill:#2c5aa0;font-weight:bold">${
          ashtakavargaValues[5] || 0
        }</text>
        <text x="30" y="230" style="font-family:Arial;font-size:12px;fill:#2c5aa0;font-weight:bold">${
          ashtakavargaValues[4] || 0
        }</text>
        <text x="80" y="155" style="font-family:Arial;font-size:12px;fill:#2c5aa0;font-weight:bold">${
          ashtakavargaValues[3] || 0
        }</text>
        <text x="30" y="80" style="font-family:Arial;font-size:12px;fill:#2c5aa0;font-weight:bold">${
          ashtakavargaValues[2] || 0
        }</text>
        <text x="93" y="58" style="font-family:Arial;font-size:12px;fill:#2c5aa0;font-weight:bold">${
          ashtakavargaValues[1] || 0
        }</text>
        ...
        setSarvashtakvargaData(updatedResponse);
      ...
      console.error("Failed to fetch Sarvashtakavarga chart:", err);
      toast.error("Failed to load Sarvashtakavarga chart. Please try again.");
    } finally {
      setIsLoadingSarvashtakvarga(false);
    }
  };
  ...
  return (
    <div className="chart-item">
      <h4 className="chart-title">Sarvashtakavarga</h4>
      <div>
        {sarvashtakvargaData?.data && (
          <SVGRenderer
            svgString={sarvashtakvargaData?.data}
...
export default SarvashtakvargaChart;

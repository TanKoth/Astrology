...
import { getD40Chart } from "../../api/Charts";
...
const D40 = ({ currentLanguage: propCurrentLanguage, onLanguageChange }) => {
  const { user } = useContext(AppContext);
  const [d40Data, setD40Data] = useState(null);
  const [isLoadingD40, setIsLoadingD40] = useState(false);
  ...
  //   document.title = `D40 Chart - ${userName}`;
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    setIsLoadingD40(true);
    try {
      const cacheKey = `d40Data_${lang}`;
      const storedData = localStorage.getItem(cacheKey);

      if (storedData && !forceRefresh) {
        try {
          const parsedData = JSON.parse(storedData);
          setD40Data(parsedData);
          setCurrentLanguage(lang);
          setIsLoadingD40(false);
          return;
        } catch (err) {
      ...
      const response = await getD40Chart(userData.user._id, apiParams);

      if (response && response.success) {
        setD40Data(response);
        localStorage.setItem(cacheKey, JSON.stringify(response));
        // toast.success("D40 chart fetched successfully", {
        //   position: "top-right",
        //   autoClose: 1000,
        // });
        setCurrentLanguage(lang);
      ...
      console.error("Failed to fetch D40 chart:", err);
      toast.error("Failed to load D40 chart. Please try again.");
    } finally {
      setIsLoadingD40(false);
    }
  };
  ...
  return (
    <div className="chart-item">
      <h4 className="chart-title">Khavedamsha (Auspicious Results - D40)</h4>
      <div>
        {d40Data?.charts && (
          <SVGRenderer svgString={d40Data?.charts} className="chart-image" />
        )}
      </div>
    </div>
  );

...
import { getD7Chart } from "../../api/Charts";
...
const D7 = ({ currentLanguage: propCurrentLanguage, onLanguageChange }) => {
  const { user } = useContext(AppContext);
  const [d7Data, setD7Data] = useState(null);
  const [isLoadingD7, setIsLoadingD7] = useState(false);
  ...
  //   document.title = `D7 Chart - ${userName}`;
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    setIsLoadingD7(true);
    try {
      const cacheKey = `d7Data_${lang}`;
      const storedData = localStorage.getItem(cacheKey);

      if (storedData && !forceRefresh) {
        try {
          const parsedData = JSON.parse(storedData);
          setD7Data(parsedData);
          setCurrentLanguage(lang);
          setIsLoadingD7(false);
          return;
        } catch (err) {
      ...
      const response = await getD7Chart(userData.user._id, apiParams);

      if (response && response.success) {
        setD7Data(response);
        localStorage.setItem(cacheKey, JSON.stringify(response));
        // toast.success("D7 chart fetched successfully", {
        //   position: "top-right",
        //   autoClose: 1000,
        // });
        setCurrentLanguage(lang);
      ...
      console.error("Failed to fetch D7 chart:", err);
      toast.error("Failed to load D7 chart. Please try again.");
    } finally {
      setIsLoadingD7(false);
    }
  };
  ...
  return (
    <div className="chart-item">
      <h4 className="chart-title">Saptamsha (Children - D7)</h4>
      <div>
        {d7Data?.charts && (
          <SVGRenderer svgString={d7Data?.charts} className="chart-image" />
        )}
      </div>
    </div>
  );

...
import { getD6Chart } from "../../api/Charts";
...
const D6 = ({ currentLanguage: propCurrentLanguage, onLanguageChange }) => {
  const { user } = useContext(AppContext);
  const [d6Data, setD6Data] = useState(null);
  const [isLoadingD6, setIsLoadingD6] = useState(false);
  ...
  //   document.title = `D6 Chart - ${userName}`;
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    setIsLoadingD6(true);
    try {
      const cacheKey = `d6Data_${lang}`;
      const storedData = localStorage.getItem(cacheKey);

      if (storedData && !forceRefresh) {
        try {
          const parsedData = JSON.parse(storedData);
          setD6Data(parsedData);
          setCurrentLanguage(lang);
          setIsLoadingD6(false);
          return;
        } catch (err) {
      ...
      const response = await getD6Chart(userData.user._id, apiParams);

      if (response && response.success) {
        setD6Data(response);
        localStorage.setItem(cacheKey, JSON.stringify(response));
        // toast.success("D6 chart fetched successfully", {
        //   position: "top-right",
        //   autoClose: 1000,
        // });
        setCurrentLanguage(lang);
      ...
      console.error("Failed to fetch D6 chart:", err);
      toast.error("Failed to load D6 chart. Please try again.");
    } finally {
      setIsLoadingD6(false);
    }
  };
  ...
  return (
    <div className="chart-item">
      <h4 className="chart-title">Shashtamsha (D6)</h4>
      <div>
        {d6Data?.charts && (
          <SVGRenderer svgString={d6Data?.charts} className="chart-image" />
        )}
      </div>
    </div>
  );

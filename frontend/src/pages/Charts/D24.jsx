...
import { getD24Chart } from "../../api/Charts";
...
const D24 = ({ currentLanguage: propCurrentLanguage, onLanguageChange }) => {
  const { user } = useContext(AppContext);
  const [d24Data, setD24Data] = useState(null);
  const [isLoadingD24, setIsLoadingD24] = useState(false);
  ...
  //   document.title = `D24 Chart - ${userName}`;
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    setIsLoadingD24(true);
    try {
      const cacheKey = `d24Data_${lang}`;
      const storedData = localStorage.getItem(cacheKey);

      if (storedData && !forceRefresh) {
        try {
          const parsedData = JSON.parse(storedData);
          setD24Data(parsedData);
          setCurrentLanguage(lang);
          setIsLoadingD24(false);
          return;
        } catch (err) {
      ...
      const response = await getD24Chart(userData.user._id, apiParams);

      if (response && response.success) {
        setD24Data(response);
        localStorage.setItem(cacheKey, JSON.stringify(response));
        // toast.success("D24 chart fetched successfully", {
        //   position: "top-right",
        //   autoClose: 1000,
        // });
        setCurrentLanguage(lang);
      ...
      console.error("Failed to fetch D24 chart:", err);
      toast.error("Failed to load D24 chart. Please try again.");
    } finally {
      setIsLoadingD24(false);
    }
  };
  ...
  return (
    <div className="chart-item">
      <h4 className="chart-title">Chaturvimshamsha (Education - D24)</h4>
      <div>
        {d24Data?.charts && (
          <SVGRenderer svgString={d24Data?.charts} className="chart-image" />
        )}
      </div>
    </div>
  );

...
import { getD10Chart } from "../../api/Charts";
...
const D10 = ({ currentLanguage: propCurrentLanguage, onLanguageChange }) => {
  const { user } = useContext(AppContext);
  const [d10Data, setD10Data] = useState(null);
  const [isLoadingD10, setIsLoadingD10] = useState(false);
  ...
  //   document.title = `D10 Chart - ${userName}`;
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    setIsLoadingD10(true);
    try {
      const cacheKey = `d10Data_${lang}`;
      const storedData = localStorage.getItem(cacheKey);

      if (storedData && !forceRefresh) {
        try {
          const parsedData = JSON.parse(storedData);
          setD10Data(parsedData);
          setCurrentLanguage(lang);
          setIsLoadingD10(false);
          return;
        } catch (err) {
      ...
      const response = await getD10Chart(userData.user._id, apiParams);

      if (response && response.success) {
        setD10Data(response);
        localStorage.setItem(cacheKey, JSON.stringify(response));
        // toast.success("D10 chart fetched successfully", {
        //   position: "top-right",
        //   autoClose: 1000,
        // });
        setCurrentLanguage(lang);
      ...
      console.error("Failed to fetch D10 chart:", err);
      toast.error("Failed to load D10 chart. Please try again.");
    } finally {
      setIsLoadingD10(false);
    }
  };
  ...
  return (
    <div className="chart-item">
      <h4 className="chart-title">Dashamsha (Profession - D10)</h4>
      <div>
        {d10Data?.charts && (
          <SVGRenderer svgString={d10Data?.charts} className="chart-image" />
        )}
      </div>
    </div>
  );

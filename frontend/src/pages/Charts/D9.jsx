...
import { getD9Chart } from "../../api/Charts";
...
const D9 = ({ currentLanguage: propCurrentLanguage, onLanguageChange }) => {
  const { user } = useContext(AppContext);
  const [d9Data, setD9Data] = useState(null);
  const [isLoadingD9, setIsLoadingD9] = useState(false);
  ...
  //   document.title = `D9 Chart - ${userName}`;
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    setIsLoadingD9(true);
    try {
      const cacheKey = `d9Data_${lang}`;
      const storedData = localStorage.getItem(cacheKey);

      if (storedData && !forceRefresh) {
        try {
          const parsedData = JSON.parse(storedData);
          setD9Data(parsedData);
          setCurrentLanguage(lang);
          setIsLoadingD9(false);
          return;
        } catch (err) {
      ...
      const response = await getD9Chart(userData.user._id, apiParams);

      if (response && response.success) {
        setD9Data(response);
        localStorage.setItem(cacheKey, JSON.stringify(response));
        // toast.success("D9 chart fetched successfully", {
        //   position: "top-right",
        //   autoClose: 1000,
        // });
        setCurrentLanguage(lang);
      ...
      console.error("Failed to fetch D9 chart:", err);
      toast.error("Failed to load D9 chart. Please try again.");
    } finally {
      setIsLoadingD9(false);
    }
  };
  ...
  return (
    <div className="chart-item">
      <h4 className="chart-title">Navamsa Chart</h4>
      <div>
        {d9Data?.charts && (
          <SVGRenderer svgString={d9Data?.charts} className="chart-image" />
        )}
      </div>
    </div>
  );

...
import { getD12Chart } from "../../api/Charts";
...
const D12 = ({ currentLanguage: propCurrentLanguage, onLanguageChange }) => {
  const { user } = useContext(AppContext);
  const [d12Data, setD12Data] = useState(null);
  const [isLoadingD12, setIsLoadingD12] = useState(false);
  ...
  //   document.title = `D12 Chart - ${userName}`;
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    setIsLoadingD12(true);
    try {
      const cacheKey = `d12Data_${lang}`;
      const storedData = localStorage.getItem(cacheKey);

      if (storedData && !forceRefresh) {
        try {
          const parsedData = JSON.parse(storedData);
          setD12Data(parsedData);
          setCurrentLanguage(lang);
          setIsLoadingD12(false);
          return;
        } catch (err) {
      ...
      const response = await getD12Chart(userData.user._id, apiParams);

      if (response && response.success) {
        setD12Data(response);
        localStorage.setItem(cacheKey, JSON.stringify(response));
        // toast.success("D12 chart fetched successfully", {
        //   position: "top-right",
        //   autoClose: 1000,
        // });
        setCurrentLanguage(lang);
      ...
      console.error("Failed to fetch D12 chart:", err);
      toast.error("Failed to load D12 chart. Please try again.");
    } finally {
      setIsLoadingD12(false);
    }
  };
  ...
  return (
    <div className="chart-item">
      <h4 className="chart-title">Dwadashamsha (Parents - D12)</h4>
      <div>
        {d12Data?.charts && (
          <SVGRenderer svgString={d12Data?.charts} className="chart-image" />
        )}
      </div>
    </div>
  );

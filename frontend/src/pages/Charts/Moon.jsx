...
import { getMoonChart } from "../../api/Charts";
...
const Moon = ({ currentLanguage: propCurrentLanguage, onLanguageChange }) => {
  ...
  //   document.title = `Moon Chart - ${userName}`;
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    ...
    try {
      const cacheKey = `moonData_${lang}`;
      const storedData = localStorage.getItem(cacheKey);

      if (storedData && !forceRefresh) {
        try {
          const parsedData = JSON.parse(storedData);
          setMoonData(parsedData);
      ...
      const response = await getMoonChart(userData.user._id, apiParams);

      if (response && response.success) {
        setMoonData(response);
        localStorage.setItem(cacheKey, JSON.stringify(response));
        // toast.success("Moon chart fetched successfully", {
        //   position: "top-right",
        //   autoClose: 1000,
        // });
        setCurrentLanguage(lang);
  ...
  return (
    <div className="chart-item">
      <h4 className="chart-title">Chandra Chart</h4>
      <div>
        {moonData?.charts && (
          <SVGRenderer svgString={moonData?.charts} className="chart-image" />
        )}
      </div>
    </div>
  );

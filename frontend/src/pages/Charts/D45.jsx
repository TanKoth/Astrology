...
import { getD45Chart } from "../../api/Charts";
...
const D45 = ({ currentLanguage: propCurrentLanguage, onLanguageChange }) => {
  const { user } = useContext(AppContext);
  const [d45Data, setD45Data] = useState(null);
  const [isLoadingD45, setIsLoadingD45] = useState(false);
  ...
  //   document.title = `D45 Chart - ${userName}`;
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    setIsLoadingD45(true);
    try {
      const cacheKey = `d45Data_${lang}`;
      const storedData = localStorage.getItem(cacheKey);

      if (storedData && !forceRefresh) {
        try {
          const parsedData = JSON.parse(storedData);
          setD45Data(parsedData);
          setCurrentLanguage(lang);
          setIsLoadingD45(false);
          return;
        } catch (err) {
      ...
      const response = await getD45Chart(userData.user._id, apiParams);

      if (response && response.success) {
        setD45Data(response);
        localStorage.setItem(cacheKey, JSON.stringify(response));
        // toast.success("D45 chart fetched successfully", {
        //   position: "top-right",
        //   autoClose: 1000,
        // });
        setCurrentLanguage(lang);
      ...
      console.error("Failed to fetch D45 chart:", err);
      toast.error("Failed to load D45 chart. Please try again.");
    } finally {
      setIsLoadingD45(false);
    }
  };
  ...
  return (
    <div className="chart-item">
      <h4 className="chart-title">Akshavedamsa (Harmonic - D45)</h4>
      <div>
        {d45Data?.charts && (
          <SVGRenderer svgString={d45Data?.charts} className="chart-image" />
        )}
      </div>
    </div>
  );

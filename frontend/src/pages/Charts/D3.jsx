...
import { getD3Chart } from "../../api/Charts";
...
const D3 = ({ currentLanguage: propCurrentLanguage, onLanguageChange }) => {
  ...
  //   document.title = `D3 Chart - ${userName}`;
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    ...
    try {
      ...
      const response = await getD3Chart(userData.user._id, apiParams);

      if (response && response.success) {
        setD3Data(response);
        localStorage.setItem(cacheKey, JSON.stringify(response));
        // toast.success("D3 chart fetched successfully", {
        //   position: "top-right",
        //   autoClose: 1000,
        // });
        setCurrentLanguage(lang);
      ...
      console.error("Failed to fetch D3 chart:", err);
      toast.error("Failed to load D3 chart. Please try again.");
    } finally {
      setIsLoadingD3(false);
    }
  };
  ...
  return (
    <div className="chart-item">
      <h4 className="chart-title">Drekkana (Sibling - D3)</h4>
      <div>
        {d3Data?.charts && (
          <SVGRenderer svgString={d3Data?.charts} className="chart-image" />
        )}
      </div>
    </div>
  );

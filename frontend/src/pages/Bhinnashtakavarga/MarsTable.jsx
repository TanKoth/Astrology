...
import { MarsTableData } from "./BhinnashtakavargaTable";
...
const MarsTable = ({
  ...
  const [bhinnashtakavargaMarsTableData, setBhinnashtakavargaMarsTableData] =
    useState(null);
  const [
    isLoadingBhinnashtakavargaMarsTable,
    setIsLoadingBhinnashtakavargaMarsTable,
  ] = useState(false);
  ...
  //   document.title = `Bhinnashtakavarga Mars Table - ${userName}`;
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    setIsLoadingBhinnashtakavargaMarsTable(true);
    try {
      const cacheKey = `bhinnashtakavargaMarsTableData_${lang}`;
      const storedData = localStorage.getItem(cacheKey);

      if (storedData && !forceRefresh) {
        try {
          const parsedData = JSON.parse(storedData);
          setBhinnashtakavargaMarsTableData(parsedData);
          setCurrentLanguage(lang);
          setIsLoadingBhinnashtakavargaMarsTable(false);
          return;
        } catch (err) {
          console.error(
            "Error parsing Bhinnashtakavarga Mars Table data:",
            err
          );
        }
      }
      ...
      const response = await getBinnashtakavargaMarsTable(
        userData.user._id,
        apiParams
      );

      if (response && response.success) {
        console.log("Bhinnashtakavarga Mars Table API Response:", response);
        setBhinnashtakavargaMarsTableData(response);
      ...
      console.error("Failed to fetch Bhinnashtakavarga Mars Table:", err);
      toast.error(
        "Failed to load Bhinnashtakavarga Mars Table. Please try again."
      );
    } finally {
      setIsLoadingBhinnashtakavargaMarsTable(false);
    }
  ...
  return (
    <MarsTableData
      bhinnashtakavargaMarsTableData={bhinnashtakavargaMarsTableData}
      isLoadingBhinnashtakavargaMarsTable={isLoadingBhinnashtakavargaMarsTable}
    />
  );
};

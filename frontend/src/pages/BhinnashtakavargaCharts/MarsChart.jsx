...
import { getBinnashtakavargaMarsTable } from "../../api/BhinnashtakavargaTable";
...
const MarsChart = ({
  ...
  const [bhinnashtakaMarsChartData, setBhinnashtakaMarsChartData] =
    useState(null);
  const [isLoadingBhinnashtakaMarsChart, setIsLoadingBhinnashtakaMarsChart] =
    useState(false);
  ...
  //   document.title = `Bhinnashtaka Mars Chart - ${userName}`;
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    setIsLoadingBhinnashtakaMarsChart(true);
    try {
      // Make cache key user-specific
      const cacheKey = `bhinnashtakaMarsChartData_${user._id}_${lang}`;
      const storedData = localStorage.getItem(cacheKey);

      if (storedData && !forceRefresh) {
        try {
          const parsedData = JSON.parse(storedData);
          setBhinnashtakaMarsChartData(parsedData);
          setCurrentLanguage(lang);
          setIsLoadingBhinnashtakaMarsChart(false);
          return;
        } catch (err) {
      ...
      // Fetch both Bhinnashtaka Mars and D1 chart data
      const [bhinnashtakaMarsResponse, d1Response] = await Promise.all([
        getBinnashtakavargaMarsTable(userData.user._id, apiParams),
        getD1Chart(userData.user._id, apiParams),
      ]);

      if (bhinnashtakaMarsResponse && bhinnashtakaMarsResponse.success) {
        // console.log(
        ...
        if (
          bhinnashtakaMarsResponse?.binnashtakavargaTable &&
          bhinnashtakaMarsResponse?.binnashtakavargaTable?.response?.Total
        ) {
          totalValues =
            bhinnashtakaMarsResponse.binnashtakavargaTable?.response?.Total;
        ...
        setBhinnashtakaMarsChartData(updatedResponse);
      ...
      console.error("Failed to fetch Bhinnashtakavarga Mars chart:", err);
      toast.error(
        "Failed to load Bhinnashtakavarga Mars chart. Please try again."
      );
    } finally {
      setIsLoadingBhinnashtakaMarsChart(false);
    }
  ...
  return (
    <div className="chart-item">
      ...
      <div>
        {bhinnashtakaMarsChartData?.data && (
          <SVGRenderer
            svgString={bhinnashtakaMarsChartData?.data}

...
const JupiterChart = ({
  ...
  const [bhinnashtakaJupiterChartData, setBhinnashtakaJupiterChartData] =
    useState(null);
  const [
    isLoadingBhinnashtakaJupiterChart,
    setIsLoadingBhinnashtakaJupiterChart,
  ] = useState(false);
  ...
  //   document.title = `Bhinnashtaka Jupiter Chart - ${userName}`;
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    setIsLoadingBhinnashtakaJupiterChart(true);
    try {
      // Make cache key user-specific
      const cacheKey = `bhinnashtakaJupiterChartData_${user._id}_${lang}`;
      const storedData = localStorage.getItem(cacheKey);

      if (storedData && !forceRefresh) {
        try {
          const parsedData = JSON.parse(storedData);
          setBhinnashtakaJupiterChartData(parsedData);
          setCurrentLanguage(lang);
          setIsLoadingBhinnashtakaJupiterChart(false);
          return;
        } catch (err) {
          console.error("Error parsing bhinnashtaka mars chart data:", err);
        }
      }
      ...
      // Fetch both Bhinnashtaka Jupiter and D1 chart data
      const [bhinnashtakaJupiterResponse, d1Response] = await Promise.all([
        getBinnashtakavargaJupiterTable(userData.user._id, apiParams),
        getD1Chart(userData.user._id, apiParams),
      ]);

      if (bhinnashtakaJupiterResponse && bhinnashtakaJupiterResponse.success) {
        // console.log(
        //   "Bhinnashtaka Jupiter API Response:",
        //   bhinnashtakaJupiterResponse
        // );
        ...
        if (
          bhinnashtakaJupiterResponse?.binnashtakavargaTable &&
          bhinnashtakaJupiterResponse?.binnashtakavargaTable?.response?.Total
        ) {
          totalValues =
            bhinnashtakaJupiterResponse.binnashtakavargaTable?.response?.Total;
        ...
        setBhinnashtakaJupiterChartData(updatedResponse);
      ...
      console.error("Failed to fetch Bhinnashtakavarga Jupiter chart:", err);
      toast.error(
        "Failed to load Bhinnashtakavarga Jupiter chart. Please try again."
      );
    } finally {
      setIsLoadingBhinnashtakaJupiterChart(false);
    }
  ...
  return (
    <div className="chart-item">
      ...
      <div>
        {bhinnashtakaJupiterChartData?.data && (
          <SVGRenderer
            svgString={bhinnashtakaJupiterChartData?.data}

...
const SaturnChart = ({
  ...
  const [bhinnashtakaSaturnChartData, setBhinnashtakaSaturnChartData] =
    useState(null);
  const [
    isLoadingBhinnashtakaSaturnChart,
    setIsLoadingBhinnashtakaSaturnChart,
  ] = useState(false);
  ...
  //   document.title = `Bhinnashtaka Saturn Chart - ${userName}`;
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    setIsLoadingBhinnashtakaSaturnChart(true);
    try {
      // Make cache key user-specific
      const cacheKey = `bhinnashtakaSaturnChartData_${user._id}_${lang}`;
      const storedData = localStorage.getItem(cacheKey);

      if (storedData && !forceRefresh) {
        try {
          const parsedData = JSON.parse(storedData);
          setBhinnashtakaSaturnChartData(parsedData);
          setCurrentLanguage(lang);
          setIsLoadingBhinnashtakaSaturnChart(false);
          return;
        } catch (err) {
          console.error("Error parsing bhinnashtaka saturn chart data:", err);
        }
      }
      ...
      // Fetch both Bhinnashtaka Saturn and D1 chart data
      const [bhinnashtakaSaturnResponse, d1Response] = await Promise.all([
        getBinnashtakavargaSaturnTable(userData.user._id, apiParams),
        getD1Chart(userData.user._id, apiParams),
      ]);

      if (bhinnashtakaSaturnResponse && bhinnashtakaSaturnResponse.success) {
        // console.log(
        ...
        if (
          bhinnashtakaSaturnResponse?.binnashtakavargaTable &&
          bhinnashtakaSaturnResponse?.binnashtakavargaTable?.response?.Total
        ) {
          totalValues =
            bhinnashtakaSaturnResponse.binnashtakavargaTable?.response?.Total;
        ...
        const updatedResponse = {
          ...bhinnashtakaSaturnResponse,
          data: newSvgStructure,
        };

        setBhinnashtakaSaturnChartData(updatedResponse);
      ...
      console.error("Failed to fetch Bhinnashtakavarga Saturn chart:", err);
      toast.error(
        "Failed to load Bhinnashtakavarga Saturn chart. Please try again."
      );
    } finally {
      setIsLoadingBhinnashtakaSaturnChart(false);
    }
  ...
  return (
    <div className="chart-item">
      ...
      <div>
        {bhinnashtakaSaturnChartData?.data && (
          <SVGRenderer
            svgString={bhinnashtakaSaturnChartData?.data}

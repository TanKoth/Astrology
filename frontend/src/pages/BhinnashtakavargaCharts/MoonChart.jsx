...
import { getBhinnashtakavargaMoonCharts } from "../../api/BhinnashtakavargaCharts";
...
const MoonChart = ({
  ...
  const fetchInsights = async (lang = "en", forceRefresh = false) => {
    ...
    try {
      ...
      const cacheKey = `moonChartData_${user._id}_${lang}`;
      ...
      if (!forceRefresh) {
        const storedMoonData = localStorage.getItem(cacheKey);
        const storedD1Data = localStorage.getItem(d1CacheKey);

        if (storedMoonData && storedD1Data) {
          try {
            const parsedMoonData = JSON.parse(storedMoonData);
            const parsedD1Data = JSON.parse(storedD1Data);
            setMoonChartData(parsedMoonData);
      ...
      // Fetch both Moon chart and D1 chart
      const [moonResponse, d1Response] = await Promise.all([
        getBhinnashtakavargaMoonCharts(userData.user._id, apiParams),
        fetchD1Chart(apiParams),
      ]);

      if (moonResponse && moonResponse.success) {
        console.log("Moon API Response for user", user._id, ":", moonResponse);
        setMoonChartData(moonResponse);
        localStorage.setItem(cacheKey, JSON.stringify(moonResponse));
        setCurrentLanguage(lang);
      }
      ...
      console.error("Failed to fetch Moon chart:", err);
      toast.error("Failed to load Moon chart. Please try again.");
    } finally {
      setIsLoadingMoon(false);
    }
  };
  ...
  const addHouseNumbersToSVG = (svgString, d1Data) => {
    ...
    const determineHouseFromCoordinates = (x, y, d1SvgString) => {
      ...
      console.log(
        `Mo is in house ${closestHouse} for user ${
          user._id
    ...
    // Function to find Moon's house in D1 chart
    const findMoonHouseInD1 = (d1ChartData) => {
      ...
      console.log(`Finding Moon position in D1 chart for user ${user._id}`);

      // Multiple patterns to find Moon in D1 chart
      const patterns = [
        /<text[^>]*x="([^"]*)"[^>]*y="([^"]*)"[^>]*>[^<]*MO[^<]*<\/text>/,
        /<text[^>]*x="([^"]*)"[^>]*y="([^"]*)"[^>]*>Mo<tspan[^>]*>[^<]*<\/tspan>/,
        /<text[^>]*x="([^"]*)"[^>]*y="([^"]*)"[^>]*>Mo<\/text>/,
      ];
      ...
      for (const pattern of patterns) {
        ...
        if (match) {
          ...
          console.log(
            `Mo found for user ${user._id} at coordinates: x=${x}, y=${y}`
          );
      ...
      console.log(
        `No Mo element found for user ${user._id}, defaulting to house 1`
      );
      return 1;
    };

    // Get Moon's house position from D1 chart
    const moonHouseInD1 = findMoonHouseInD1(d1Data);
    console.log(
      `Final result for user ${user._id}: Mo is in house ${moonHouseInD1} in D1 chart`
    );

    // Calculate rotation: if Moon is in house X in D1,
    // then Moon chart house 1 should be positioned where house X would be
    const rotationOffset = moonHouseInD1 - 1;
  ...
  return (
    <div className="chart-item">
      ...
      <div>
        {moonChartData?.moonData && (
          <SVGRenderer
            svgString={addHouseNumbersToSVG(
              moonChartData?.moonData,
              d1ChartData
            )}

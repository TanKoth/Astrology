...
// const dasha = async (req, res,apiEndPoint, suggestionType) => {
//   try{
...
//     const dashaReport = await getDasha(birthDetails,apiEndPoint);

//     res.status(200).json({success: true, message: `Report for ${suggestionType} retrieved successfully`, dashaReport: dashaReport});
//   }catch(err){
//     console.error(`Error in creating report for ${suggestionType}:`, err.message)
//     res.status(400).json({success: false, message: `Error in creating report for ${suggestionType}`, error: err.message});
//   }
// }

// const getDasha = async (birthDetails, endpoint) =>{
//   try{
...
//     const baseUrl = `https://api.jyotishamastroapi.com/api/dasha/${endpoint}`;
...
const getDashaReport = async(req,res) => {
  try{
      const userId = req.params.userId;
      ...
      const data = await client.get('https://api.prokerala.com/v2/astrology/dasha-periods', {
          'ayanamsa': 1,
